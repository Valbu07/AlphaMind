...
router.get('/',verificarToken,autorizaciondeRoles(['Administrador']),async (req,res) => {
    try {
...
router.get('/:num_documento',verificarToken,autorizaciondeRoles(['Administrador']), async (req,res) => {  
    try {
...
router.get('/cargo/todos' , verificarToken,autorizaciondeRoles(['Funcionario', 'Administrador']), async (req,res) => {
    try {
        const data = await controlador.cargo();
        respuesta.success(req,res,data,200);
    } catch(error) {
        respuesta.error(req,res,'Error al obtener el funcionario y su Cargo',500);
    }
});
...
router.post('/agregar',verificarToken,autorizaciondeRoles(['Administrador']) ,async (req, res) => {
    try {
...
router.put('/actualizar/:num_documento', verificarToken, autorizaciondeRoles(['Administrador']), async (req, res) => {
  try {
    const data = {
...
router.delete('/:num_documento', verificarToken,autorizaciondeRoles(['Administrador']),async (req,res) => {
    try {

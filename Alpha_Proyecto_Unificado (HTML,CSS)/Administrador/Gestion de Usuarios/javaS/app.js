...
document.querySelector('#contrasena .formulario').addEventListener('submit', function(e) {
  e.preventDefault();
  var id = Number(this.querySelector('select').value);
  var pass = this.querySelector('input').value;
  if (id && pass) actualizarUsuario(id, { password: pass });
  this.reset();
});
